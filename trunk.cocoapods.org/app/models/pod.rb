...
module Pod
  module TrunkApp
    class Pod < Sequel::Model
      ...
      # Query a pod by name, only including deleted pods if `include_deleted` is
      # true.
      #
      def self.query_by_name(term, include_deleted: false)
        if include_deleted
          filter(:name.ilike('%' + term + '%'))
        else
          filter(:name.ilike('%' + term + '%')).filter(:deleted => false)
        end
      end
